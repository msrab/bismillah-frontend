...
describe('Street API', () => {
  describe('POST /api/streets', () => {
    ...
    it('ne crée pas une rue sans nom', async () => {
      const res = await request(app)
        .post('/api/streets')
        .set('Authorization', `Bearer ${token}`)
        .send({ cityId: city.id });
    ...
    it('ne crée pas une rue sans cityId', async () => {
      const res = await request(app)
        .post('/api/streets')
        .set('Authorization', `Bearer ${token}`)
  ...
  describe('GET /api/streets/city/:cityId', () => {
    it('retourne toutes les rues d\'une ville', async () => {
      await Street.create({ name: 'Avenue de l\'Opéra', cityId: city.id });
      const res = await request(app)
        .get(`/api/streets/city/${city.id}`);
  ...
  describe('GET /api/streets/:id', () => {
    it('retourne une rue par ID', async () => {
      // S'assure que la rue existe
      let rivoli = await Street.findOne({ where: { name: 'Rue de Rivoli' } });
      if (!rivoli) {
